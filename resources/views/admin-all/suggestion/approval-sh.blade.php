...
@section('title', 'Persetujuan SH')

@section('admin-content')
@php
    $summary = $shQueue['summary'] ?? [];
    $rows = $shQueue['rows'] ?? [];
    ...
    $statusTone = static function (?string $status): string {
        ...
        return match ($status) {
            'VERIFIED_GL_QCC' => 'pending',
            'APPROVED_SH' => 'verified',
            'REJECTED_SH' => 'rejected',
            default => '',
        };
    };
...
<div class="glv-stats">
    ...
    <div class="glv-stat revision">
        <span class="glv-stat-icon">Σ</span>
        <small>Total Queue</small>
        <strong>{{ number_format($summary['total'] ?? 0) }}</strong>
        <span>Riwayat data pada tahap Persetujuan SH</span>
    </div>
...
<section class="glv-card">
    <div class="glv-card-head">
        <div>
            <h2>Queue Persetujuan SH</h2>
            <p>
                Klik NO SS / Review Detail untuk memeriksa data dan menjalankan SETUJUI / TOLAK pada akun SH/ADMIN yang aktif.
            </p>
        </div>
            ...
            <tbody>
                @forelse($rows as $row)
                    <tr>
                        <td>
                            <a
                                href="{{ route('admin-all.suggestion.detail', ['noSs' => $row['NO_SS'], 'from' => 'sh']) }}"
                                class="glv-no"
                            >
                                {{ $row['NO_SS'] }}
                            </a>
                        ...
                        <td>
                            <a
                                href="{{ route('admin-all.suggestion.detail', ['noSs' => $row['NO_SS'], 'from' => 'sh']) }}"
                                class="glv-action {{ ($canReviewSh && strtoupper(trim((string) ($row['STATUS'] ?? ''))) === 'VERIFIED_GL_QCC') ? 'primary' : '' }}"
                            >
                                Lihat Detail
                            </a>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="glv-empty">
                            Belum ada data pada queue Persetujuan SH.
                        </td>
                    </tr>
                @endforelse
            </tbody>
