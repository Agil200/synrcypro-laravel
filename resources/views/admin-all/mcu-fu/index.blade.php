...
{{-- SYNRGYPRO MCU & FU DASHBOARD V6 — JABATAN DROPDOWN FILTER --}}
...
<style>
    ...
    |--------------------------------------------------------------------------
    | Header/sidebar/footer tetap milik admin-all.layout.
    | Di halaman ini:
    | - Judul + action tetap di atas
    | - Filter tetap di atas
    | - KPI tetap di atas
    | - HANYA area chart yang scroll
    |--------------------------------------------------------------------------
    */
    ...
    .mfi-page > .aa-page-title,
    .mfi-page > .mfi-alert,
    .mfi-page > .mfi-filter-card,
    .mfi-page > .mfi-kpis {
        flex: 0 0 auto;
    }

    .mfi-dashboard-scroll {
        display: grid;
        min-height: 0;
        flex: 1 1 auto;
        gap: 9px;
    ...
    .mfi-filter {
        display: grid;
        grid-template-columns: 120px 150px 210px 94px 58px;
        align-items: end;
    ...
    */
    ...
    .mfi-specialist-fill {
        display: block;
        width: var(--specialist-width);
        height: 100%;
        min-width: 2px;
        border-radius: inherit;
        background: var(--specialist-color);
        transition: filter .15s ease, transform .15s ease;
        transform-origin: left center;
    }

    .mfi-specialist-row:hover .mfi-specialist-fill {
        filter: saturate(1.25);
        transform: scaleX(1.01);
    }

    .mfi-specialist-value {
        color: #233b52;
        font-size: 8px;
        font-weight: 900;
        text-align: right;
    ...
    */
    ...
    .mfi-job-vbar {
        display: grid;
        grid-template-rows: 18px 145px 32px;
        gap: 5px;
        align-items: end;
        color: inherit;
        text-align: center;
    ...
    .mfi-job-vbar:hover .mfi-job-fill {
        opacity: .85;
        transform: scaleY(1.02);
    }
...
<div class="mfi-page">
    ...
    <div class="mfi-filter-card">
        ...
        >
            ...
            <div class="mfi-field">
                <label for="mfiJabatan">Jabatan</label>
                <select
                    name="jabatan"
                    id="mfiJabatan"
                ...
                >
                    ...
                    @foreach (($filters['jabatan_options'] ?? []) as $jabatanOption)
                        <option
                            value="{{ $jabatanOption }}"
                            @selected(($filters['jabatan'] ?? '') === $jabatanOption)
                        >
                            {{ $jabatanOption }}
                        </option>
                    @endforeach
    ...
    <div class="mfi-dashboard-scroll">
    ...
    <div class="mfi-grid">
        ...
        <div class="mfi-card">
            <div class="mfi-card-head">
                <div>
                    <h2>Tahap Follow Up</h2>
                    <p>
                        Jumlah karyawan yang sudah terisi di FU 1, FU 2, dan FU 3.
                    </p>
                </div>
    ...
    {{-- ================================================================
         DETAIL TUJUAN / SPESIALIS FOLLOW UP 1, 2, 3
    ================================================================= --}}
    <div class="mfi-specialist-grid">
        @php
            $specialistCharts = [
                [
                    'stage' => 1,
                    'title' => 'Follow Up 1 — Tujuan Spesialis',
                    'subtitle' => 'Tujuan pemeriksaan pada FOLLOW UP 1.',
                    'items' => $dashboard['follow_up_1_detail'],
                    'offset' => 0,
                ],
                [
                    'stage' => 2,
                    'title' => 'Follow Up 2 — Tujuan Spesialis',
                    'subtitle' => 'Tujuan pemeriksaan pada FOLLOW UP 2.',
                    'items' => $dashboard['follow_up_2_detail'],
                    'offset' => 3,
                ],
                [
                    'stage' => 3,
                    'title' => 'Follow Up 3 — Tujuan Spesialis',
                    'subtitle' => 'Tujuan pemeriksaan pada FOLLOW UP 3.',
                    'items' => $dashboard['follow_up_3_detail'],
                    'offset' => 6,
                ],
            ];
        @endphp

        @foreach ($specialistCharts as $chart)
            <div class="mfi-card">
                <div class="mfi-card-head">
                    <div>
                        <h2>{{ $chart['title'] }}</h2>
                ...
                <div class="mfi-card-body">
                    @if (empty($chart['items']))
                        <div class="mfi-empty">
                            Belum ada data Follow Up {{ $chart['stage'] }}.
                        </div>
                    @else
                        <div class="mfi-specialist-bars">
                            @foreach ($chart['items'] as $index => $item)
                                ...
                                <a
                                    ...
                                    class="mfi-specialist-row"
                                    title="Klik untuk melihat Follow Up {{ $chart['stage'] }} — {{ $item['label'] }}"
                                >
                                    <span
                                        class="mfi-specialist-label"
                                        title="{{ $item['label'] }}"
                                    >
                                        {{ $item['label'] }}
                                    </span>

                                    <span class="mfi-specialist-track">
                                        <span
                                            class="mfi-specialist-fill"
                                            style="
                                                --specialist-width: {{ $item['percent'] }}%;
                                                --specialist-color: {{ $color }};
                                            "
                                        ></span>
                                    </span>

                                    <span class="mfi-specialist-value">
                                        {{ number_format($item['total']) }}
                                    </span>
                                </a>
                            @endforeach
    ...
    <div class="mfi-card">
        <div class="mfi-card-head">
            <div>
                ...
                <p>
                    ...
                    Klik batang untuk membuka datanya.
                </p>
            </div>
        ...
        <div class="mfi-card-body">
            ...
            @else
                <div class="mfi-job-chart-wrap">
                    <div class="mfi-job-vbars">
                        @foreach ($dashboard['jabatan'] as $index => $item)
                            ...
                            <a
                                ...
                                title="Klik untuk melihat {{ $item['label'] }}"
                            >
                                <span class="mfi-job-value">
                                    {{ number_format($item['total']) }}
                                </span>

                                <span class="mfi-job-track">
                                    <span
                                        class="mfi-job-fill"
                                        style="
                                            --job-height: {{ $item['percent'] }}%;
                                            --job-color: {{ $color }};
                                        "
                                    ></span>
                                </span>

                                <span class="mfi-job-label">
                                    {{ $item['label'] }}
                                </span>
                            </a>
                        @endforeach
